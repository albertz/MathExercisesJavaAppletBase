...
class OperatorTree implements Comparable<OperatorTree> {
    ...
    static abstract class PositionInfo {
    	int absolutePos = 0;
    	PositionInfo.BetweenEntities parent = null;
    	
    	abstract OperatorTree.PositionInfo next();
    	int prefixLen() { return 0; }
    	int postfixLen() { return 0; }
    	PositionInfo.BetweenEntities currentTreePos() { return parent; }
    	
    	OperatorTree.PositionInfo nextInStack() {
    		if(parent == null) return null;
    		PositionInfo.BetweenEntities pos = new BetweenEntities();
        ...
        static class StringPos extends OperatorTree.PositionInfo {
        	OperatorTree.RawString rs;
        	int pos;            	

        	@Override OperatorTree.PositionInfo next() {
				if(pos >= rs.content.length()) return nextInStack();
				PositionInfo.StringPos pos = new StringPos();
        ...
        static class BetweenEntities extends OperatorTree.PositionInfo {
        	OperatorTree ot;
        	...
        	@Override PositionInfo.BetweenEntities currentTreePos() { return this; }
        	@Override OperatorTree.PositionInfo next() {
				...
				OperatorTree.PositionInfo pos = null;
				if(nextEntity instanceof OperatorTree.RawString) {
					PositionInfo.StringPos spos = new StringPos();
					spos.rs = (OperatorTree.RawString) nextEntity;
					spos.pos = 0;
					pos = spos;
				}
				else {
					PositionInfo.BetweenEntities spos = new BetweenEntities();
        ...
        static OperatorTree.PositionInfo start(OperatorTree ot) {
        	PositionInfo.BetweenEntities pos = new BetweenEntities();
        	pos.ot = ot;
        	return pos;
        }
        
        static OperatorTree.PositionInfo posAt(OperatorTree ot, int pos) {
        	OperatorTree.PositionInfo p = start(ot);
        	while(p.absolutePos < pos) {
        		OperatorTree.PositionInfo next = p.next();
    ...
    OperatorTree.PositionInfo posAt(int pos) { return PositionInfo.posAt(this, pos); }
    
    Iterator<OperatorTree.PositionInfo> positionIterator() {
    	return new Iterator<OperatorTree.PositionInfo>() {
    		OperatorTree.PositionInfo next = PositionInfo.start(OperatorTree.this);        		
			public boolean hasNext() { return next != null; }
			public OperatorTree.PositionInfo next() {
				if(!hasNext()) throw new NoSuchElementException();
				OperatorTree.PositionInfo res = next;
				next = next.next();
				return res;
			}
			public void remove() { throw new UnsupportedOperationException(); }        	
    ...
    Iterable<OperatorTree.PositionInfo> position() {
    	return new Iterable<OperatorTree.PositionInfo>() {
    		public Iterator<OperatorTree.PositionInfo> iterator() { return positionIterator(); }
		};
    }
